...
class BULLET_SOFT_EXPORT	btSoftBody : public btCollisionObject
{
public:
	...
	struct	Node : Feature
	{
		...
		bool operator>( const Node& rhs ) const  // linyuan add for compiler pass
		{
			return m_x.x() > rhs.m_x.x();
		}
		...
		bool operator==( const Node& rhs ) const
		{
			return (m_x.x() - rhs.m_x.x()) < 0.00001f;
		}
	};
	/* Link			*/ 
	struct	Link : Feature
	{
		...
		bool operator>( const Link& rhs ) const  // linyuan add for compiler pass
		{
			return m_rl > rhs.m_rl;
		}
	...
	struct	Face : Feature
	{
		...
		btDbvtNode*				m_leaf;			// Leaf data
		bool operator>( const Face& rhs ) const  // linyuan add for compiler pass
		{
			return m_ra > rhs.m_ra;
		}
...
//				printf("impulse.m_drift = %f,%f,%f\n",impulse.m_drift.getX(),impulse.m_drift.getY(),impulse.m_drift.getZ());
	...
	tNodeIndexOfFaceArray   m_linkNodeIndex;
	tNodeIndexOfFaceArray   m_faceNodeIndex;
	...
	{
		return (btCollisionObject *)this;
	}

	void CollisionShapeSetMargin(float margin)
	{
		if( m_collisionShape )
	...
	btVector3 GetNodeIndexFromFace(Face& face)
	{
