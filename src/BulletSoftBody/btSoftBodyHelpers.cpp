...
btSoftBody*		btSoftBodyHelpers::CreateFromConvexHull(btSoftBodyWorldInfo& worldInfo,   
		tVector3Array& verticesArray,
		bool randomizeConstraints)
{
	int nvertices = verticesArray.size();
    btVector3* vertices = new btVector3[nvertices];
	for(int index=0;index<nvertices;index++)
	{
		vertices[index].m_floats[0] = verticesArray[index].x();
		vertices[index].m_floats[1] = verticesArray[index].y();
		vertices[index].m_floats[2] = verticesArray[index].z();
		vertices[index].m_floats[3] = verticesArray[index].w();
	}
